...
Latest version no longer uses touch features; instead, it uses three switches for better user interface. Features are nearly the same. Find the latest version in the folder: MPU6050_WLED_withThree_Switches. Note: must use the new variables.h file; as variable names have changed. First time use; edit variable values to match your network and configuration. 
