/*
    ...
    "MPU6050_WLED_withFive_Switches.ino  04/09/2023 0300 EDT"  Adding Pause for sending of URLs.

    Developed by William Lucid with an assist from OpenAI's ChatGPT 03/10/2023 @ 01:00 EST.  Sketch was developed to generate varialbles for effects, Intensity, and color 
    ...
    Tested and loged continous "waving" of "paddle" that has the MPU6050 attached; to get  MIN and MAX values of range.  So far; best range of 
    the goal 0-255 has been with the following settings:
    
    MPU6050 settings:  16 G, 250 degrees, 5 Hz    
    
    Named the "paddle/wand;" Chromawand.  Chromawand MPU6050 to generate variables for the motion function.  noMotion function generate random FX, and FP variables for custom URLs 
	to send via HTTPClient to the Athom ESP32 controller running WLED.
...
void setup() {
  ...
  Serial.print("\n\n\nMPU6050_WLED_withFive_Switches.ino  04/09/2023 0300 EDT\n\n\n");
...
void swTest() {
  ...
  if (sw == 0) {
    ...
    switchTest = digitalRead(switch1Pin);
    ...
    switchTest = digitalRead(switch2Pin);
    while (switchTest == HIGH) {
      switchTest = digitalRead(switch2Pin);
    }
    ...
    switchTest = digitalRead(interruptPin);
    while (switchTest == HIGH) {
      switchTest = digitalRead(interruptPin);
    }
